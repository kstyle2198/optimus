...
def 블록데이터전처리(블록원데이터):
    ...
    블록데이터["가로"] = 블록데이터["가로"].apply(lambda x: int(np.round(x,0)))
    블록데이터["세로"] = 블록데이터["세로"].apply(lambda x: int(np.round(x,0)))
    블록데이터["세로"] = 블록데이터[["블록", "세로"]].apply(E32C세로길이조정하기, axis=1)
    블록데이터["사이즈"] = 블록데이터[["가로", "세로"]].apply(블록사이즈튜플만들기, axis=1)
    블록데이터["면적"] = 블록데이터.eval("가로*세로")
    블록데이터["최대길이"] = 블록데이터["사이즈"].apply(lambda x: int(max(x)))
    블록데이터["최소길이"] = 블록데이터["사이즈"].apply(lambda x: int(min(x)))
...
def 중조립블록_반출사이즈_체크(blk_id, 정반리스트):
    ...
    if 블록종류 == "중조립" and (("C1" in 정반리스트)or("C2" in 정반리스트)):
        ...
        if 작은길이 > 14 or 큰길이 >= 15:    #작은길이 >= 13 or 큰길이 >= 15
            for 정반 in ["C1", "C2"]:
                try:
                    정반리스트.remove(정반)
                except:
...
# 특정 값만 남기고 다 지운후에 그 결과를 다시 각 해당일 surface에 더해준다..
            ...
            if len(확정_검토그룹)!=0:
                ...
                if len(미정_중조_검토그룹) != 0:   # 미정 중조 블록 검토 개시
                    for 중조_id in 미정_중조_검토그룹:
                        ...
                        # 중조는 가능한 C계열 정반이 있으면, C계열 정반에 우선깐다.
