    ...
    with st.expander("🧪 생산계획수립"):
        ...
        if st.button("📈 생산계획수립"):
            ...
            for _ in range(len(st.session_state['수정블록리스트'])):
                ...
                st.markdown(f"통합 가능정반 min value : {min(가능정반_dict.values())}")   ## 날짜가 제일 빠른 정반 찾기
                ...
                st.markdown(f"적격한 검토정반: {랜덤최선정반}, size_capa: {size_capa}, target_size:{target_size}, 잔여면적비율 :{잔여면적비율}")
