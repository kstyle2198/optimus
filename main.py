...
if __name__ == "__main__":
    ...
    for _ in range(len(블록리스트)):
        ...
        for 블록ID in 확정_PAIR_블록:
            ...
            if 정반명:
                확정_PAIR_블록_정반계열.append(정반명[:1])
                확정_PAIR_블록_정반계열 = list(set(확정_PAIR_블록_정반계열))

        미정_PAIR_블록종류 = []
        for 블록ID in 미정_PAIR_블록:
            블록종류 = 블록데이터[블록데이터["blk_id"]==블록ID]["블록종류"].values[0]
            if 정반명:
                미정_PAIR_블록종류.append(블록종류)

        # print(f"확정_PAIR_블록: {확정_PAIR_블록}, 확정_PAIR_블록_정반계열:{확정_PAIR_블록_정반계열} | 미정_PAIR_블록: {미정_PAIR_블록}, 미정_PAIR_블록종류:{미정_PAIR_블록종류}")

        중조대상, 대조대상, 대조타입 = [], [], []
        for id, 종류 in zip(미정_PAIR_블록, 미정_PAIR_블록종류):
            if 종류 == "중조립":
                중조대상.append(id)
            else:
                타입 = 블록데이터[블록데이터["blk_id"]==id]["타입"].values[0]
                대조대상.append(id)
                대조타입.append(타입)
        print()
        print(f">>> 확정_블록: {확정_PAIR_블록} | 중조대상: {중조대상} | 대조대상:{대조대상}, 대조타입:{대조타입}")


        '''
        - 확정블록 있냐? (있으면 확정블록의 정반계열 확인)

        - 중조블록 있냐?
        --  중조블록 순차적으로 깔고..(1번 중조블록의 정반계열이 후속 중조블록의 정반에 영향을 미친다.)
        --  중조블록 있으면서, 대조PAIR 블록 있냐?
        --  중조블록 있으면서, 대조SINGLE 블록 있냐?
        --  중조블록 있으면서, 대조블록 없냐?
        
        - 중조블록 없냐?
        -- 중조블록 없으면서, 대조PAIR 블록 있냐?
        -- 중조블록 없으면서, 대조SINGLE 블록 있냐?
        '''

        if len(확정_PAIR_블록) != 0:   # PAIR관계 확정블록이 있으면 확정정반계열을 확인해라.
            확정정반계열 = 확정_PAIR_블록_정반계열  # list --> 예: ["C", "D"]
            if "D" in 확정정반계열:
                중조가능정반계열 = ["C", "D"]
                대조가능정반계열 = ["D"]
            elif "E" in 확정정반계열:
                ...
                중조가능정반계열 = ["C", "D", "E"]
                대조가능정반계열 = ["D", "E"]
            # print(f"확정정반계열:{확정정반계열}, 중조가능정반계열:{중조가능정반계열}, 대조가능정반계열:{대조가능정반계열}")


            if len(중조대상) != 0:  # 중조블록 있으면..
                print()
                print(">>> (확정PAIR블록 있는 상태) 중조블록 배치 개시")
                중조블록모음1, 중조정반모음1, 중조날짜모음1 = [], [], []
                ...
                for blk_id in 중조대상:
                    ...
                    # 물리조건
                    반출한계가능정반 = 반출한계사이즈체크(blk_id)
                    정반최대길이가능정반 = 정반최대길이체크(blk_id, 반출한계가능정반)
                    중조립체크정반 = 중조립블록_반출사이즈_체크(blk_id, 정반최대길이가능정반)
                    블록종류체크정반 = 블록종류체크(blk_id, 중조립체크정반)
                    중조가능정반 = [x for x in 블록종류체크정반 if x[:1] in 중조가능정반계열]
                    print(f"물리조건 체크후 정반: {블록종류체크정반} | 중조가능정반계열:{중조가능정반계열} | 중조가능정반:{중조가능정반}")

                    prev_레이아웃달력 = 레이아웃달력.copy()
                    임시달력모음, 임시정반모음, 임시날짜모음 = [], [], []
                    for 정반 in 중조가능정반:
                        레이아웃달력, 최선정반, 최종배치일 = update_레이아웃달력(레이아웃달력, 정반, blk_id, 정반데이터, 블록데이터, 조기착수금지, 블록간격, 후퇴계수)
                    ...
                    ### Earliest 방식
                    # earliest_date = min(임시날짜모음)
                    # earliest_index = 임시날짜모음.index(earliest_date)
                    # 최선정반, 최선날짜, 최선달력 = 임시정반모음[earliest_index], 임시날짜모음[earliest_index], 임시달력모음[earliest_index]

                    ### Random 방식
                    random_number = randint(1, len(임시날짜모음)) - 1
                    최선정반, 최선날짜, 최선달력 = 임시정반모음[random_number], 임시날짜모음[random_number], 임시달력모음[random_number]

                    중조블록모음1.append(blk_id)
                    중조정반모음1.append(최선정반)
                    중조날짜모음1.append(최선날짜)
                    중조정반모음계열1 = [x[:1]for x in 중조정반모음1]
                    중조정반모음계열1 = list(set(중조정반모음계열1))
                print(f"☆☆☆ 중조블록모음1:{중조블록모음1}, 중조정반모음1:{중조정반모음1}, 중조날짜모음1:{중조날짜모음1}") #☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆



                if len(대조대상) != 0: # 확정PAIR 있고, 중조대상 있는 상태에서 대조도 있다면...
                    # 대조가능정반은 중조가능정반에서 C계열 제외 (D계열 또는 E계열 가능)
                    if "D" in 중조정반모음계열1:
                        대조가능정반계열 = ["D"]
                    elif "E" in 중조정반모음계열1:
                        대조가능정반계열 = ["E"]
                    ...
                    # 대조블록 스펙 -  PAIR 체크
                    if len(대조대상) == 2 and "S" in 대조타입 and "P" in 대조타입:  # 이러면 PAIR 대조
                        ...
                        for blk_id in 대조대상:
                            ...
                            중조립체크정반 = 중조립블록_반출사이즈_체크(blk_id, 정반최대길이가능정반)
                            블록종류체크정반 = 블록종류체크(blk_id, 중조립체크정반)
                            대조가능정반 = [x for x in 블록종류체크정반 if x[:1] in 대조가능정반계열]
                            print(f"물리조건 체크후 정반: {블록종류체크정반} | 대조가능정반계열:{대조가능정반계열} | 대조가능정반:{대조가능정반}")
                ...
                else:    # 확정PAIR 있고, 중조대상 있는 상태에서 대조는 없다면...중조는 이미 앞에서 배치했으니.. PASS하면 될 듯
                    pass
        ...
        else:  # PAIR 관계 있는 확정블록이 없으면...
            중조가능정반계열 = ["C", "D", "E"]
            대조가능정반계열 = ["D", "E"]
            print()
            print(f"PAIR관계 확정블록 없음, 중조가능정반계열:{중조가능정반계열}, 대조가능정반계열:{대조가능정반계열}")

            if len(중조대상) != 0:  # 중조블록 있으면..
                print(">>> (확정PAIR블록 없는 상태) 중조블록 배치 개시")
                중조블록모음1, 중조정반모음1, 중조날짜모음1 = [], [], []
                ...
                for blk_id in 중조대상:
                    ...
                    if len(임시날짜모음) != 0: # 배치가능한 날짜가 있음면...
                        ...
                        print("배치가능한 날짜가 없습니다.")


                if len(대조대상) != 0: # 확정PAIR 없고, 중조대상 있는 상태에서 대조도 있다면...
                    # 대조가능정반은 중조가능정반에서 C계열 제외 (D계열 또는 E계열 가능)
                    대조블록모음1, 대조정반모음1, 대조날짜모음1 = [], [], []
                    ...
                    if len(대조대상) == 2 and "S" in 대조타입 and "P" in 대조타입:  # 이러면 PAIR 대조
                        ...
                        for blk_id in 대조대상:
                            ...
                            if len(임시날짜모음) != 0: # 배치가능한 날짜가 있음면...
                                ...
                                대조블록모음1.append(blk_id)
                                대조정반모음1.append(최선정반)
                                대조날짜모음1.append(최선날짜)
                            
                                print(f"☆☆☆ 대조PAIR- 블록모음1:{대조블록모음1}, 정반모음1:{대조정반모음1}, 날짜모음1:{대조날짜모음1}") #☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆
                            else:
                                print("배치가능한 날짜가 없습니다.")
                ...
                else:    # 확정PAIR 없고, 중조대상 있는 상태에서 대조는 없다면...중조는 이미 앞에서 배치했으니.. PASS하면 될 듯
                    pass
